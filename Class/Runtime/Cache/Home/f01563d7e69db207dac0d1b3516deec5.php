...
<section class="page-body">
	...
	<ol class="breadcrumb">
	  <li>首页</li>
	  <?php if(empty($oneName)): ?><!--判断oneName（一级标题）是否为空，为空则模板显示为空-->
	  	<?php else: ?>
	  	<!--不为空则模板显示$oneName（一级标题）-->
	 	 <li><?php echo ($oneName); ?></li><?php endif; ?>
	  <?php if(empty($category_article)): ?><!--判断category_article（根据category_id取出的文章数据）是否为空，为空则模板显示暂无文章-->
	  	<li>暂无文章</li>
	  	<?php else: ?>
<!--判断category_article（根据category_id取出的文章数据）是否为空，
不为空则模板显示$category_article[0]['category_name']（一级或二级标题）-->
	 	 <li><?php echo ($category_article[0]['category_name']); ?></li><?php endif; ?>
	</ol>
	<div class="article-list">
	 <?php if(is_array($category_article)): $i = 0; $__LIST__ = $category_article;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><dl class="article">
		...
		<dd class="good">
			<div class="row">
				...
				<div class="col-md-9" style="float: right;">
					<i class="glyphicon glyphicon-heart" style="color: grey;font-size: 1.2em;"></i><!-- <?php echo ($k['update_time']); ?> -->120
